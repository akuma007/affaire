...
if (isset($_POST['submit'])) { // Assuming you have a "submit" button with name="submit"
    ...
    if (isset($_POST['csrf']) && $_POST['csrf'] === $_SESSION['csrf']) {
        ...
        // Prepare statement 
        $sql = "INSERT INTO défendeur (id_défendeur, nom_défendeur, adresse_défendeur, qualité_défendeur, email, téléphone)
                 VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = $db->prepare($sql);
        ...
        $id_défendeur= $_POST['id_défendeur'];
        $nom_défendeur = $_POST['nom_défendeur'];
        $adresse_défendeur = $_POST['adresse_défendeur'];
        $qualité_défendeur = $_POST['qualité_défendeur'];
        $email = $_POST['email'];
...
<body>
  <form method="post">
    ...
    <label>رقم المدعي عليه</label>
    <input type="text" name="id_défendeur">

    <label>اسم المدعي عليه</label>
    <input type="text" name="nom_défendeur">

    <label>عنوان المدعي عليه</label>
    <input type="text" name="adresse_défendeur">

    <label>صفة المدعي عليه</label>
    <input type="text" name="qualité_défendeur">
